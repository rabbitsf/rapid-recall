  ...
  return (
    <section className="max-w-3xl mx-auto flex flex-col items-center animate-in fade-in duration-500 select-none">
      <div className="w-full flex justify-between items-center mb-8">
        <button onClick={onBack} className="p-3 bg-white rounded-full shadow-sm text-slate-500 hover:text-indigo-600 transition-colors touch-manipulation"><ArrowLeft size={24} /></button>
      ...
      <div className="w-full aspect-[4/3] sm:aspect-[16/9] max-w-2xl" style={{ perspective: '1000px' }}>
        <div onClick={() => setFlipped(f => !f)} className="relative w-full h-full transition-transform duration-500 cursor-pointer touch-manipulation" style={{ transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
          ...
          <div className="absolute inset-0 bg-indigo-600 text-white rounded-3xl shadow-lg flex items-center justify-center p-8 md:p-12" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
      ...
      <div className="flex items-center gap-8 mt-12">
        <button onClick={() => go(-1)} className="w-16 h-16 flex items-center justify-center bg-white rounded-full shadow-md text-slate-600 hover:text-indigo-600 active:scale-95 transition-all touch-manipulation"><ArrowLeft size={28} /></button>
        <button onClick={() => go(1)} className="w-16 h-16 flex items-center justify-center bg-white rounded-full shadow-md text-slate-600 hover:text-indigo-600 active:scale-95 transition-all touch-manipulation"><ArrowLeft size={28} className="rotate-180" /></button>
      </div>
    </section>
  )
}
