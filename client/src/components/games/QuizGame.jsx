  ...
  if (finished) {
    ...
    return (
      <div className="max-w-lg mx-auto bg-white rounded-3xl p-8 sm:p-12 text-center shadow-sm border border-slate-100 animate-in zoom-in-95">
        <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-gold-100 text-gold-600 mb-6"><Trophy size={48} /></div>
        <h2 className="text-3xl font-bold text-slate-800 mb-2">Quiz Complete!</h2>
        <p className="text-slate-500 mb-4">You scored {score} out of {questions.length}</p>
        <div className="text-5xl font-black bg-clip-text text-transparent bg-gradient-to-r from-crimson-500 to-gold-500 mb-8">{pct}%</div>
        {missed.length > 0 && (
        ...
        <div className="flex gap-4">
          ...
          <button onClick={init} className="flex-1 px-6 py-4 bg-gold-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 touch-manipulation"><RefreshCw size={20} /> Try Again</button>
        </div>
      </div>
    )
  }
    ...
    <section className="max-w-2xl mx-auto animate-in fade-in select-none">
      ...
      <div className="space-y-4">
        {q.options.map(opt => {
          ...
          let cls = 'bg-white border-2 border-slate-200 hover:border-crimson-300 hover:bg-crimson-50 text-slate-700'
          let Icon = null
          if (checked) {
          ...
          } else if (sel) { cls = 'bg-crimson-50 border-crimson-500 text-crimson-800 shadow-sm scale-[1.01]' }
          return (
      ...
      {!checked && selectedId && (
        <button onClick={check} className="w-full mt-8 py-5 bg-crimson-600 hover:bg-crimson-700 text-white font-bold text-lg rounded-2xl shadow-md transition-colors touch-manipulation">Check Answer</button>
      )}
    </section>
  )
}
