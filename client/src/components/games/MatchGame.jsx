...
export default function MatchGame({ set, onBack, onCreateMissedSet }) {
  ...
  if (won) return (
    <div className="max-w-2xl mx-auto text-center bg-white p-12 rounded-3xl shadow-sm border border-emerald-100 animate-in zoom-in-95">
      ...
      <p className="text-xl text-slate-600 mb-8">Cleared in <span className="font-bold text-indigo-600">{elapsed}s</span> with <span className="font-bold text-indigo-600">{moves}</span> moves.</p>
      {missed.length > 0 && (
  ...
  return (
    <section className="max-w-5xl mx-auto animate-in fade-in select-none">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4">
          <div className="bg-white px-4 py-2 rounded-xl shadow-sm flex items-center gap-2 font-medium text-slate-700 border border-slate-100"><Clock size={18} className="text-indigo-500" /> {elapsed}s</div>
          <div className="bg-white px-4 py-2 rounded-xl shadow-sm flex items-center gap-2 font-medium text-slate-700 border border-slate-100"><Layers size={18} className="text-indigo-500" /> {moves}</div>
        </div>
      </div>
      <div className={`grid gap-3 sm:gap-4 ${tiles.length <= 8 ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-3 md:grid-cols-4 lg:grid-cols-5'}`}>
        {tiles.map(tile => {
          ...
          let cls = 'bg-white border-2 border-slate-200 text-slate-700 hover:border-indigo-300 hover:bg-slate-50'
          ...
          else if (isSelected) cls = 'bg-indigo-50 border-indigo-500 text-indigo-700 scale-[1.02] shadow-md'
          return (
